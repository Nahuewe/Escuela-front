...
export const useAfiliadoStore = () => {
  ...
  const startLoadingAfiliado = async (page) => {
    try {
      const response = await edjaApi.get(`/personas?page=${page}`)
  ...
  const startEditAfiliado = async (id) => {
    try {
      ...
      const response = await edjaApi.get(`/personas/${id}`)
      const { data } = response.data
  ...
  const startDeleteAfiliado = async () => {
    try {
      const currentPage = paginate?.current_page || 1
      await edjaApi.delete(`/personas/${activeAfiliado.id}`)
  ...
  const startSearchAfiliado = async (search, page = 1) => {
    try {
      const response = await edjaApi.get(`/buscar-persona?query=${search}&page=${page}`)
      const { data, meta } = response.data
      dispatch(handleAfiliado({ data, meta }))
    } catch (error) {
      console.log(error)
